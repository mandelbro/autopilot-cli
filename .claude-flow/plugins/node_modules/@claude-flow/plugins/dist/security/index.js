 ...
 */
export function validateArray(input, itemValidator, options) {
 ...
 */
export function validateEnum(input, allowedValues) {
    if (typeof input !== 'string')
        return null;
...
export function validatePath(inputPath, options) {
    if (typeof inputPath !== 'string')
        return null;
    if (inputPath.length === 0 || inputPath.length > MAX_PATH_LENGTH)
        return null;
    // Normalize the path
 ...
 */
export async function safePathAsync(baseDir, ...segments) {
    const resolved = path.resolve(baseDir, ...segments);
    try {
...
export function validateCommand(command, options) {
    ...
    if (blocked.has(cmd))
        return null;
    if (!allowed.has(cmd) && allowed.size > 0)
        return null;
 ...
 */
export function createResourceLimiter(limits) {
    ...
    return {
        ...
        async enforce(fn) {
            ...
            return Promise.race([
                fn(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Execution time limit exceeded')), config.maxExecutionTime)),
            ]);
        },
    };
}
