...
export class ProviderRegistry extends EventEmitter {
    ...
    getBest(options) {
        ...
        if (options?.capabilities) {
            candidates = candidates.filter(e => options.capabilities.every(cap => e.provider.definition.capabilities.includes(cap)));
        }
        // Filter by model support
        if (options?.model) {
            candidates = candidates.filter(e => e.provider.definition.models.includes(options.model));
        }
    ...
    async tryFallback(request, failedProvider) {
        ...
        for (const fallbackName of fallbackChain) {
            if (fallbackName === failedProvider)
                continue;
...
export class ProviderFactory {
     ...
     */
    static createCustom(definition) {
        return { ...definition };
    }
}
